...
2. 设置环境变量或使用内置API密钥
3. 运行程序：python zhenjiu_extractor_doubao.py
...
# 内置API密钥配置
DOUBAO_API_KEY = "1f18966f-ed49-4ef4-aee5-a920cecc2652"  # 您的豆包API密钥
DOUBAO_MODEL = "doubao-seed-1-6-250615"  # 您的模型名
...
try:
    ...
    print("   或者：pip install volcengine-python-sdk[ark] -i https://pypi.tuna.tsinghua.edu.cn/simple/")
    sys.exit(1)
...
class ZhenjiuEntityExtractor:
    ...
    def __init__(self, api_key: str = None):
        ...
        self.api_key = api_key or DOUBAO_API_KEY or os.getenv('ARK_API_KEY')
        if not self.api_key:
            raise ValueError(
                "API密钥未配置！请检查程序内置密钥或设置环境变量 ARK_API_KEY"
            )
        ...
        try:
            ...
            raise ValueError(f"豆包客户端初始化失败: {e}")
...
def check_environment():
    ...
    try:
        from volcenginesdkarkruntime import Ark
        print(f"   volcengine-python-sdk[ark]包: 已安装")
    except ImportError:
        print(f"   volcengine-python-sdk[ark]包: 未安装")
        return False
...
def main():
    ...
    print("模型：豆包 (doubao-seed-1-6-250615)")
    print("版本：4.0 (简化版)")
    print("="*50)
    ...
    if not check_environment():
        ...
        print("1. 安装依赖包：")
        print("   pip install volcengine-python-sdk[ark]")
        print("   或使用国内镜像：pip install volcengine-python-sdk[ark] -i https://pypi.tuna.tsinghua.edu.cn/simple/")
    ...
    try:
        ...
        # 保存结果到文件
        output_file = f"extraction_result_{selected_doc['filename'][:-4]}_doubao.json"
        output_data = {
