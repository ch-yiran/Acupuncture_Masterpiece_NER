...
2. 运行程序：python zhenjiu_extractor.py
...
# 内置API密钥配置
DASHSCOPE_API_KEY = "sk-693c076dafbe4d34bf43a3f18e272201"
...
try:
    ...
    print("   或者：pip install dashscope -i https://pypi.tuna.tsinghua.edu.cn/simple/")
    sys.exit(1)
...
class ZhenjiuEntityExtractor:
    ...
    def __init__(self, api_key: str = None):
        ...
        if not self.api_key:
            raise ValueError(
                "API密钥未配置！请检查程序内置密钥或设置环境变量 DASHSCOPE_API_KEY"
            )
    ...
    def build_optimized_prompt(self, text: str) -> str:
        ...
        prompt = f"""你是《针灸大成》专家。请从古文中抽取四类实体，要求全面且准确。
...
{{
    ...
    def call_qianwen_api(self, prompt: str) -> str:
        """调用通义千问API"""
        try:
    ...
    def extract_entities(self, text: str, doc_title: str = "") -> Dict[str, Any]:
        ...
        try:
            response = self.call_qianwen_api(prompt)
...
def check_environment():
    ...
    try:
        import dashscope
        print(f"   dashscope包: 已安装")
    except ImportError:
        print(f"   dashscope包: 未安装")
        return False
...
def main():
    ...
    if not check_environment():
        ...
        print("1. 安装依赖包：")
        print("   pip install dashscope")
        print("   或使用国内镜像：pip install dashscope -i https://pypi.tuna.tsinghua.edu.cn/simple/")
